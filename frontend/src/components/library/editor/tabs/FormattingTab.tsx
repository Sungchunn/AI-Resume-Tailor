...
type FormattingSection = "style" | "font" | "spacing";

/**
 * FormattingTab - Style controls for the resume editor
 *
 * Features:
...
export function FormattingTab() {
  ...
  const [activeSection, setActiveSection] = useState<FormattingSection>("style");
...
/**
 * Style preset selector section with auto-fit toggle
 */
...
function SpacingSection({
  ...
  const inputClass = `w-full px-2 py-1.5 text-sm border border-input rounded focus:ring-2 focus:ring-ring focus:border-transparent bg-background ${
    disabled ? "bg-muted opacity-60 cursor-not-allowed" : ""
  }`;

  const inputClassLg = `w-full px-3 py-2 text-sm border border-input rounded-md focus:ring-2 focus:ring-ring focus:border-transparent bg-background ${
    disabled ? "bg-muted opacity-60 cursor-not-allowed" : ""
  }`;
